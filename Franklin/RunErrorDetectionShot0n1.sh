...
TRANSFORMERS_CACHE=./HFCache HF_HOME=./HFCache CUDA_VISIBLE_DEVICES=0 vllm serve "Qwen/Qwen2-VL-72B-Instruct-AWQ" --dtype=half --tensor-parallel-size 1 --limit-mm-per-prompt image=3 --gpu_memory_utilization 0.95 --port 8101 > EDapi.log 2>&1 &
TRANSFORMERS_CACHE=./HFCache HF_HOME=./HFCache CUDA_VISIBLE_DEVICES=1 vllm serve "Qwen/Qwen2-VL-72B-Instruct-AWQ" --dtype=half --tensor-parallel-size 1 --limit-mm-per-prompt image=3 --gpu_memory_utilization 0.95 --port 8102 > EDapi.log 2>&1 &
...
while ! curl -s http://localhost:8101/v1/models; do
    echo "Waiting for API to be ready..."
    sleep 5
done

# Check if the API is up
while ! curl -s http://localhost:8102/v1/models; do
    echo "Waiting for API to be ready..."
    sleep 5
done
...
python3 RunErrorDetection.py --path /fastwork/psalvador/JHU/data/AbdomenAtlas1.0Projections/projections_AtlasBench_beta_pro/ --port 8101 \
--csv_path ErrorDetectionAtlasBench0Shot.csv --continuing --organ_list all --examples 0 \
--dice_list /fastwork/psalvador/JHU/data/AbdomenAtlas1.0Projections/ >> ErrorDetectionAtlasBench0Shot.log 2>&1 &

python3 RunErrorDetection.py --path /fastwork/psalvador/JHU/data/AbdomenAtlas1.0Projections/projections_AtlasBench_beta_pro/ --port 8102 \
--csv_path ErrorDetectionAtlasBench1Shot.csv --continuing --organ_list all --examples 1 \
--dice_list /fastwork/psalvador/JHU/data/AbdomenAtlas1.0Projections/ >> ErrorDetectionAtlasBench1Shot.log 2>&1
