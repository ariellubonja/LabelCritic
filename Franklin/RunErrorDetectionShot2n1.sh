...
TRANSFORMERS_CACHE=./HFCache HF_HOME=./HFCache CUDA_VISIBLE_DEVICES=0 vllm serve "Qwen/Qwen2-VL-72B-Instruct-AWQ" --dtype=half --tensor-parallel-size 1 --limit-mm-per-prompt image=4 --gpu_memory_utilization 0.95 --port 8103 > EDapi.log 2>&1 &
TRANSFORMERS_CACHE=./HFCache HF_HOME=./HFCache CUDA_VISIBLE_DEVICES=1 vllm serve "Qwen/Qwen2-VL-72B-Instruct-AWQ" --dtype=half --tensor-parallel-size 1 --limit-mm-per-prompt image=11 --gpu_memory_utilization 0.95 --port 8104 > EDapi.log 2>&1 &
...
# Check if the API is up
while ! curl -s http://localhost:8103/v1/models; do
    echo "Waiting for API to be ready..."
    sleep 5
done

# Check if the API is up
while ! curl -s http://localhost:8104/v1/models; do
    echo "Waiting for API to be ready..."
    sleep 5
done
...
python3 RunErrorDetection.py --path /fastwork/psalvador/JHU/data/AbdomenAtlas1.0Projections/projections_AtlasBench_beta_pro/ --port 8103 \
--csv_path ErrorDetectionAtlasBench2Shot.csv --continuing --organ_list all --examples 2 \
--dice_list /fastwork/psalvador/JHU/data/AbdomenAtlas1.0Projections/  >> ErrorDetectionAtlasBench2Shot.log 2>&1 &

python3 RunErrorDetection.py --path /fastwork/psalvador/JHU/data/AbdomenAtlas1.0Projections/projections_AtlasBench_beta_pro/ --port 8104 \
--csv_path ErrorDetectionAtlasBench10Shot.csv --continuing --organ_list all --examples 10 \
--dice_list /fastwork/psalvador/JHU/data/AbdomenAtlas1.0Projections/  >> ErrorDetectionAtlasBench10Shot.log 2>&1
